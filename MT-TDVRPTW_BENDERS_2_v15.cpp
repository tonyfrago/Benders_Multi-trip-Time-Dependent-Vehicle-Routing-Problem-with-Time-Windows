...
char* FilePath_Results = "G:/Antonis/PhD_Examples/MT-TDVRPTW/MT-TDVRPTWResults-Small/T15-5-2/BENDERS_2_DYNAMIC_SEC";//G:/Antonis/PhD_Examples
...
int CreateAddSECS(IloEnv env, IloModel modelMaster_ptr, int Sub_ptr, int** SubtoursNodes_ptr, int* NodesInSubtour_ptr, IloNumVarMatrix5x5 X_ijkrm, int* IsSubtourToBeEliminated_l, int loop) {
	...
	IloRangeMatrix2x2 SEC_kr(env, 0);
	for (int l = 0; l < Sub_ptr; l++) {
		if (IsSubtourToBeEliminated_l[l] == 1) {
			for (k = 0; k < kmax; k++) {
				IloRangeArray SEC_r(env, 0);
				for (r = 0; r < rmax; r++) {
					IloExpr expr101(env);
					for (int n = 0; n < NodesInSubtour_ptr[l]; n++) {
						i = SubtoursNodes_ptr[l][n];
						for (int d = 0; d < NodesInSubtour_ptr[l]; d++) {
							if (d != n) {
								j = SubtoursNodes_ptr[l][d];
								//if (i != j) {
								for (m = 0; m < mmax; m++) {
					...
					SECNumber++;
					sprintf(MasterCut, "SEC(iter%d, SECiter%d, subtour%d, k%d,r%d)", loop, SECIterations, SECNumber, k, r);
	...
	cout << "SUBTOUR ELIMINATION CONSTRAINTS HAVE BEEN ADDED" << endl;
	return 0;
}
	...
	for (k = 0; k < kmax; k++) {
		for (r = 0; r < rmax; r++) {
			for (i = 0; i < imax + 1; i++) {
				for (j = 0; j < jmax + 1; j++) {
					for (m = 0; m < mmax; m++) {
						UpperBound = UpperBound + Distance_ij[i][j] * X_ijkrmValue[i][j][k][r][m];
					}
				}
			}
		}
...
int Generate_New_Sequence_Solutions(int MultiSol, bool* GeneratedNewSolution, int* NodeChanged, int k_ptr) {
...
NewSolution:;
	...
	//	for (r = 0; r < rmax; r++) {
	//		for (i = 0; i < imax; i++) {
	//			if (DualValueCon10a_ZZY_ikr[i][k_ptr][r] != 0) {
	//				for (j = 1; j < jmax + 1; j++) {
	//					for (m = 0; m < mmax - 1; m++) {
	//						if (X_ijkrmValue[i][j][k_ptr][r][m] != 0) {
	//							m_init = m;
	...
	//						StartOver1:;
	//							for (j = 1; j < jmax + 1; j++) {
	//								if (X_ijkrmValue[i][j][k_ptr][r][m_init] != 0) {
	//									X_ijkrmValue[i][j][k_ptr][r][m_init] = 0;
	//									X_ijkrmValue[i][j][k_ptr][r][m_init + 1] = 1;
	//									i = j;
	//									goto StartOver1;
	//								}
	//							}
	...
	//	for (r = 0; r < rmax; r++) {
	//		for (i = 0; i < imax; i++) {
	//			if (DualValueCon10b_ZZY_ikr[i][k_ptr][r] != 0) {
	//				for (j = 1; j < jmax + 1; j++) {
	//					for (m = 1; m < mmax; m++) {
	//						if (X_ijkrmValue[i][j][k_ptr][r][m] != 0) {
	//							m_init = m;
	...
	//						StartOver2:;
	//							for (j = 1; j < jmax + 1; j++) {
	//								if (X_ijkrmValue[j][i][k_ptr][r][m_init] != 0) {
	//									X_ijkrmValue[j][i][k_ptr][r][m_init] = 0;
	//									X_ijkrmValue[j][i][k_ptr][r][m_init - 1] = 1;
	//									i = j;
	//									goto StartOver2;
	//								}
	//							}
...
int BendersIteration(IloEnv env, IloModel modelMaster_ptr, IloModel modelSlave_ptr, IloNumVarMatrix5x5 X_ijkrm, IloNumVarMatrix3x3 Y_ikr, IloNumVarArray Heta_k, IloRangeMatrix2x2 Con2_YX_kr, IloRangeMatrix2x2 Con3_YX_kr, IloRangeMatrix3x3 Con4a_YX_ikr, IloRangeMatrix3x3 Con4b_YX_ikr, IloRangeArray Con5_Y_i, IloRangeMatrix2x2 Con6_Y_kr, IloRangeMatrix4x4 Con15_XX_ikrm, IloRangeMatrix5x5 Con16_X_ijkrm, IloRangeArray ConAuxiliary_i, IloNumVarMatrix2x2 ZZ_ir, IloRangeMatrix4x4 Con9_ZZZX_ijrm, IloRangeMatrix2x2 Con10a_ZZY_ir, IloRangeMatrix2x2 Con10b_ZZY_ir, IloRangeMatrix4x4 Con11a_ZX_ijrm, IloRangeMatrix4x4 Con11b_ZX_ijrm, IloRangeArray Con12_ZZY_n, IloRangeArray Con13_ZZY_r, IloRangeArray Con14_ZZY_r, IloRangeArray BendersCuts, IloNum OptDualValues, IloNumArray FeasDualValues, char* FilePath_Results_ptr, int* loop_iter, IloModel modelAuxSlave, IloNumVarMatrix3x3 X_aux_ijm, IloNumVarArray ZZ_aux_i, IloRangeArray Con2_aux_YX_n, IloRangeArray Con3_aux_YX_n, IloRangeArray Con4a_aux_YX_i, IloRangeArray Con4b_aux_YX_i, IloRangeMatrix3x3 Con9_aux_ZZZX_ijm, IloRangeArray Con10a_aux_ZZY_i, IloRangeArray Con10b_aux_ZZY_i, IloRangeMatrix3x3 Con11a_aux_ZX_ijm, IloRangeMatrix3x3 Con11b_aux_ZX_ijm, IloRangeArray Con14_aux_ZZY_n, IloRangeMatrix2x2 AuxFeasCut_kr) {
	...
	while (Gap > epsilon && loop < 10000 && ((clock() - start) / CLOCKS_PER_SEC) < MaxBendersDuration) {
		...
		SolveMasterAgain:;
			...
			//if (((clock() - start) / CLOCKS_PER_SEC) > MaxBendersDuration) goto SolveSlave;
			startPrint = clock();
	...
	SolveSlave:;
		...
		for (k = 0; k < kmax; k++) {
			if (SlaveIsFeasible[k] == 0 && MultiSol < 2 * MaxNoOfMultiSol) {//
				bool GeneratedNewSolution = false;
				status = Generate_New_Sequence_Solutions(MultiSol, &GeneratedNewSolution, &NodeChanged, k);
				if (status != 0) {
					Found_Error("Generate_New_Sequence_Solutions");
					return -1;
				}
				//cout << "NodeChanged= " << NodeChanged << endl;
				MultiSol++;
		...
		*/
		...
		//if (UpperBoundGlobal < Big_M) StopTime = (MaxBendersDuration - (clock() - start) / CLOCKS_PER_SEC) / 2.0;
...
int main(int argc, char** argv)
{
	...
	for (int i_Problem = 20; i_Problem < 21; i_Problem++) {
		//if (i_Problem == 20 || i_Problem == 21 || i_Problem == 22 || i_Problem == 25 || i_Problem == 26) {//if (i_Problem == 20 || i_Problem == 24 || i_Problem == 26) {
		...
		try {
			...
			status = do_master(env, modelMaster, X_ijkrm, Y_ikr, U_ikr, Heta_k, Con2_YX_kr, Con3_YX_kr, Con4a_YX_ikr, Con4b_YX_ikr, Con5_Y_i, Con6_Y_kr, Con15_XX_ikrm, Con16_X_ijkrm, Con17_XX_krm, Con18_Y_kr, Con19_Y_kr, Con20_X_kr, Con20a_XX_krm, Con21_XX_ikr, Con22_X_ijm, Con23_Xw_krm, Con24_hX_n, Con24a_hX_m, Con25_hX_n, Con26_YY_ijkr, Con27_hX_n, Con28_Y_kr, Con29_XX_ijkr, Con30_XXX_ijlkr, Con31_XX_ijlm, Con32_XY_kr, ConAuxiliary_i, ConAuxiliary2_n);//ConSEC_ijkr,
			if (status != 0) {
